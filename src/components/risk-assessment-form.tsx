...
import { emptyRiskAssessment, emptyRiskHazard, riskManuals, riskProgress, riskSteps, type RiskAssessment, type RiskHazard } from '@/domain/risk-assessment';
...
export function RiskAssessmentFields({ stored, demo, requestedStep, cursor, workplace, resource }: { stored?: RiskAssessment; demo: boolean; requestedStep?: string; cursor?: string | null; workplace: string; resource: string }) {
  ...
  function remember(next: RiskJourneyPosition) {
  ...
  function field(key: RiskField, label: string, hint?: string, type: 'text' | 'date' | 'area' = 'area') {
    const id = `risk-${key}`;
    const common = { id, value: risk[key], onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setRisk(current => ({ ...current, [key]: e.target.value })), 'aria-describedby': hint ? `${id}-hint` : undefined };
    return <label htmlFor={id}>{label}{hint ? <span className="helper" id={`${id}-hint`}>{hint}</span> : null}{type === 'area' ? <textarea {...common} rows={2} maxLength={1500}/> : <input {...common} type={type} maxLength={1500}/>}</label>;
  }
  function hazardField(hazard: RiskHazard, key: HazardField, label: string, hint?: string, type: 'text' | 'date' | 'area' = 'area') {
    ...
    const common = { id, value: hazard[key], onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => updateHazard(hazard.id, key, e.target.value), 'aria-describedby': hint ? `${id}-hint` : undefined };
    return <label htmlFor={id}>{label}{hint ? <span className="helper" id={`${id}-hint`}>{hint}</span> : null}{type === 'area' ? <textarea {...common} rows={2} maxLength={1500}/> : <input {...common} type={type} maxLength={key === 'owner' || key === 'verifier' ? 120 : 1500}/>}</label>;
  }
  ...
  return <div className="risk-workflow">
    <input type="hidden" name="risk_assessment" value={JSON.stringify(risk)}/>
    <button type="button" className="journey-help-button" aria-pressed={!guided} onClick={() => setGuided(value => !value)}>{guided ? '익숙하다면 · 전체 항목 직접 입력' : '한 번에 하나씩 안내받기'}</button>
    <div className="risk-intro"><p>위험성평가는 <strong>일하다 다칠 수 있는 원인을 찾고, 실제로 줄이는 과정</strong>입니다. 아래 순서로 진행하고 확인한 내용부터 저장하세요.</p><p className="helper">현재 입력 기준 {progress.readyCount}/7단계의 기록이 갖춰졌습니다. 법적 이행 완료나 현장 안전을 판정하는 표시가 아닙니다.</p>{progress.unresolvedCount ? <p className="risk-attention" role="status">확인·개선이 남은 위험요인 {progress.unresolvedCount}건</p> : null}</div>
    <div className="journey-saved-progress"><span>저장된 기록 준비</span><strong>{savedProgress?.readyCount ?? 0}/7단계</strong><progress aria-label="저장된 기록 준비" max={7} value={savedProgress?.readyCount ?? 0}/><small>현재 입력은 저장한 뒤 지도에 반영돼요.</small></div>
    <details className="journey-step-picker" open={!guided}><summary>전체 7단계에서 이동하기 · 현재 {step + 1}단계</summary><nav className="risk-step-nav" aria-label="위험성평가 진행 단계">{progress.steps.map((item, i) => <button type="button" key={item.id} aria-current={step === i ? 'step' : undefined} onClick={() => move(i)}><span>{i + 1}</span><span>{item.title}<small>{item.ready ? '현재 입력 갖춤' : '작성·확인 필요'}</small></span></button>)}</nav></details>
    <section className="risk-step-panel" aria-label={`${step + 1}단계 ${riskSteps[step].title}`}>
      <p className="eyebrow">지금 보는 위치 · {riskSteps[step].title}</p><div className="section-heading"><h3 ref={heading} tabIndex={-1}>{riskChapters[step][chapter]}</h3><span className="muted">{chapter + 1} / {riskChapters[step].length}</span></div><p>{riskSteps[step].action}</p>{guided ? <RiskScene step={step}/> : null}<JourneyHelp title={riskHelp[step][0]}>{step === 2 ? <RiskStory/> : <p>{riskHelp[step][1]}</p>}</JourneyHelp>
      ...
      {step >= 2 && step <= 5 ? <>
        ...
        {risk.hazards.map((h, i) => <fieldset hidden={missingHazard || (guided && h.id !== position.hazardId)} onFocusCapture={() => { if (!guided && h.id !== position.hazardId) selectHazard(h.id); }} className="risk-hazard" key={h.id}><legend>위험요인 {i + 1}{h.hazard ? ` · ${h.hazard.slice(0, 45)}` : ''}</legend>
          {step === 2 ? <>{hazardField(h, 'work', '어떤 작업인가요?')}{hazardField(h, 'location', '어디에서 하나요?', undefined, 'text')}{hazardField(h, 'hazard', '무엇 때문에 어떻게 다칠 수 있나요?')}{hazardField(h, 'people', '누가 영향을 받을 수 있나요?', '해당 작업자뿐 아니라 청소·정비·도급 작업자 등 노출될 수 있는 사람을 살펴보세요.')}{hazardField(h, 'existingControls', '현재 하고 있는 안전조치', '실제 조치를 적으세요. 없다면 없다고 적고, 확인하지 못했다면 아직 판단하지 마세요.')}<button type="button" className="text-link" onClick={() => setRisk({ ...risk, hazards: risk.hazards.filter(item => item.id !== h.id) })}>이 위험요인 입력 취소</button></> : null}
    ...
    <div className="risk-step-actions"><button type="button" className="button secondary" disabled={step === 0 && chapter === 0} onClick={() => chapter > 0 ? move(step, chapter - 1) : move(step - 1, riskChapters[step - 1].length - 1)}>이전 단계</button>{next ? <><button className="button primary" name="journey_next" value={riskJourneyCursor({ ...position, ...next })}>저장하고 다음</button><button type="button" className="text-link" onClick={() => move(next.step, next.chapter)}>다음 단계</button></> : <a className="text-link" href="#evidence-records">저장 후 증빙 연결하기 →</a>}<span className="helper">모르는 내용은 비워둘 수 있어요. 다음 단계로 이동만 하면 저장되지 않아요.</span></div>{positionError ? <p role="status" className="helper">보던 위치를 저장하지 못했어요. 입력 내용은 임시 저장 버튼으로 보관해주세요.</p> : null}
  </div>;
}
