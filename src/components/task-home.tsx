...
export async function TaskHome({ demo, selected, setup = false, focus, view = 'today', browse = false }: { demo: boolean; selected?: string; setup?: boolean; focus?: string; view?: 'today' | 'map'; browse?: boolean }) {
  ...
  const riskCursor = (view === 'map' || !gated) && riskTask && data.workplace ? await journeyState(demo, data.workplace.id, riskTask.id) : null;
  return <>
    <header className="home-heading"><h1>{view === 'map' ? '전체 관리 지도' : gated ? '사업장 정보 입력' : '오늘 할 일'}</h1>{view === 'map' ? <p>분야를 고르면 기록과 다음 행동을 볼 수 있어요.</p> : null}</header>
    {data.workplace ? <>
      ...
      {view === 'map' ? <JourneyMap riskCursor={riskCursor} tasks={activeTasks} sources={activeSources} actions={actions} profile={profile} workplace={data.workplace.id} base={base}/> : gated ? <><BusinessProfile key={`${data.workplace.id}:${focus ?? ''}`} demo={demo} workplace={data.workplace.id} profile={profile} back={back} focus={focus} initialStep={Number(cursor?.split(':')[1] ?? 0)}/><div className="home-setup-links"><Link className="text-link" href={`${base}/map?workplace=${encodeURIComponent(data.workplace.id)}`}>먼저 전체 흐름 보기 · 전체 지도 →</Link></div></> : <>
        <JourneyHome tasks={data.tasks} sources={data.sources} actions={actions} profile={profile} workplace={data.workplace.id} base={base} riskResume={riskTask ? { taskId: riskTask.id, cursor: riskCursor } : undefined}/>
        <details className="home-workplace-details" aria-label="확인한 사업장 정보"><summary>{profile.facts.businessName || data.workplace.name} · 사업장 정보</summary><div><p>{industries.find(i => i.id === profile.industry)?.label ?? '업종 확인 필요'} · 직접고용 {profile.headcount === null ? '미확인' : `${profile.headcount}명`}</p><p>{profile.facts.actualWork}</p><Link className="text-link" href={setupHref}>사업장 정보 변경</Link></div></details>
