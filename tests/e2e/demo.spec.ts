...
async function persona(page: Page, name: string) {
  ...
  await expect(page).toHaveURL(name.includes('검토자') ? /\/ops$/ : /\/app$/);
}
...
test('회원 제출 → 운영자 보완 → 새 버전 재제출 → 검토, 내부 메모·다른 고객 차단', async ({ page }) => {
  ...
  await start(page);
  await page.getByRole('link', { name: '자료 준비하기', exact: true }).click();
  ...
  await persona(page, 'A 고객 담당자'); await page.goto('/app/obligations/facility-1');
  ...
  await persona(page, 'B 고객 담당자'); await page.goto('/app/obligations/facility-1');
  await expect(page.getByRole('heading', { name: '이 자료를 찾을 수 없습니다.' })).toBeVisible();
  await page.goto('/app'); await persona(page, '미배정 검토자'); await page.goto(reviewPath);
  await expect(page.getByRole('heading', { name: '이 자료를 찾을 수 없습니다.' })).toBeVisible();
...
test('360px·확대 상당 폭·접근성·공개 긴급 경로', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '오늘도, 하나씩 챙겨볼까요?' })).toBeVisible();
  ...
  await zoomPage.goto('http://127.0.0.1:3000/app');
